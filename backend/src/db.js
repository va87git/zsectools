...
// MAPPER
// ─────────────────────────────────────────────────────────────────────────────
// for each "element" (user) to be mapped, it assigns their transactions to a set
// of candidate roles using a greedy set-cover algorithm:
// at each round, it selects the role that covers the most residual transactions;
// in case of a tie, the one with the smaller delta; in case of an absolute tie,
// the alphabetically smaller one.
// Whatever remains uncovered is "MISSING", and any extra transactions brought
// by the winning role (optional) are "EXTRA".
...
// populates MAP_ELEMENT_TO_MAP_TCODES
// based on real usage statistics (YR_STATISTIC_SLIM).
...
// ── Algorithm ──────────────────────────────────────────────────────────────
// For each element: a greedy loop that, at each iteration, selects from the
// candidate roles the one covering the most uncovered transactions (the one
// covering more wins; in case of a tie, the one with fewer "extra" tcodes,
// i.e., smaller delta = total - covered; in case of an absolute tie, the
// role with the alphabetically smaller name — same logic as the original C#).
// The tcodes assigned to the winner become COVERED and are removed from the
// residual; the winner's tcodes not requested by the element become EXTRA
// (if required). This repeats until no role covers anything anymore; the
// final residual is MISSING.
